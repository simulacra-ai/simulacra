...
function get_create_mock(sdk: OpenAI): Mock {
  return sdk.chat.completions.create as unknown as Mock;
}
...
describe("getCloudflareGatewayBaseURL", () => {
  it("constructs the correct URL with default provider", () => {
    const url = getCloudflareGatewayBaseURL({
      accountId: "my-account",
      gatewayName: "my-gateway",
    });
    expect(url).toBe("https://gateway.ai.cloudflare.com/v1/my-account/my-gateway/openai");
  });

  it("constructs the correct URL with a custom provider", () => {
    const url = getCloudflareGatewayBaseURL({
      accountId: "acct-123",
      gatewayName: "prod-gw",
      provider: "workers-ai",
    });
    expect(url).toBe("https://gateway.ai.cloudflare.com/v1/acct-123/prod-gw/workers-ai");
  });
});
...
describe("createCloudflareClient", () => {
  it("creates a client that is an object (basic smoke test)", () => {
    const client = createCloudflareClient("test-key", {
      accountId: "acct",
      gatewayName: "gw",
    });
...
// ---------------------------------------------------------------------------
// CloudflareProvider – construction & clone
// ---------------------------------------------------------------------------

describe("CloudflareProvider – construction", () => {
  it("stores the config and exposes an empty context_transformers array by default", () => {
    const sdk = make_mock_sdk(async_stream());
    const provider = new CloudflareProvider(sdk, { model: "gpt-4o" });
  ...
  it("accepts custom context_transformers", () => {
    ...
    const provider = new CloudflareProvider(sdk, { model: "gpt-4o" }, [transformer]);
    expect(provider.context_transformers).toHaveLength(1);
  });

  it("clone() returns a new provider with the same config", () => {
    const sdk = make_mock_sdk(async_stream());
    const provider = new CloudflareProvider(sdk, { model: "gpt-4o", max_tokens: 100 });
    const clone = provider.clone();
    expect(clone).not.toBe(provider);
    expect(clone).toBeInstanceOf(CloudflareProvider);
  });
});
...
describe("CloudflareProvider – system message handling", () => {
  it("sends system prompt with role 'system'", async () => {
    ...
    const [call_params] = get_create_mock(sdk).mock.calls[0];
...
describe("CloudflareProvider – streaming text response", () => {
...
describe("CloudflareProvider – tool call streaming", () => {
...
describe("CloudflareProvider – cancellation", () => {
...
describe("CloudflareProvider – error handling", () => {
...
describe("CloudflareProvider – tool definitions", () => {
...
describe("CloudflareProvider – user message conversion", () => {
...
describe("CloudflareProvider – assistant message raw content compatibility", () => {
  it("handles raw content with model_kind 'cloudflare'", async () => {
    const stream = async_stream(
      make_chunk({ choices: [make_choice(0, { role: "assistant", content: "ok" }, "stop")] }),
    );
    const sdk = make_mock_sdk(stream);
    ...
    const messages: Message[] = [
      ...
      {
        role: "assistant",
        content: [{ type: "raw", model_kind: "cloudflare", data: '{"custom_field":"value"}' }],
      },
    ];
...
describe("CloudflareProvider – usage tracking", () => {
...
describe("CloudflareProvider – request hooks", () => {
  it("calls before_request and request_raw before streaming starts", async () => {
    const stream = async_stream(
      make_chunk({ choices: [make_choice(0, { role: "assistant", content: "hi" }, "stop")] }),
    );
    const sdk = make_mock_sdk(stream);
    const provider = new CloudflareProvider(sdk, { model: "gpt-4o", max_tokens: 256 });
    const receiver = make_receiver();
