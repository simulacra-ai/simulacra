...
/**
 * Configuration options for the Cloudflare AI Gateway provider.
 */
export type CloudflareProviderConfig = Record<string, unknown> & {
  /** The model identifier to use (e.g., "@cf/meta/llama-3.1-8b-instruct"). */
  model: string;
...
/**
 * Configuration for connecting to a Cloudflare AI Gateway.
 */
...
/**
 * Constructs the Cloudflare AI Gateway base URL for the given gateway configuration.
 *
 * @param config - The gateway configuration.
 * @returns The base URL string for the Cloudflare AI Gateway.
 */
export function getCloudflareGatewayBaseURL(config: CloudflareGatewayConfig): string {
  const provider = config.provider ?? "openai";
  return `https://gateway.ai.cloudflare.com/v1/${config.accountId}/${config.gatewayName}/${provider}`;
}

/**
 * Creates an OpenAI SDK client configured to route through a Cloudflare AI Gateway.
 *
 * @param apiKey - The API key for the upstream provider (e.g., your OpenAI API key).
 * @param gatewayConfig - Configuration for the Cloudflare AI Gateway.
 * @returns A configured OpenAI client instance pointed at the Cloudflare AI Gateway.
 */
export function createCloudflareClient(
  apiKey: string,
  gatewayConfig: CloudflareGatewayConfig,
): OpenAI {
  return new OpenAI({
    apiKey,
    baseURL: getCloudflareGatewayBaseURL(gatewayConfig),
  });
}

/**
 * Model provider implementation for Cloudflare AI Gateway's OpenAI-compatible chat completion API.
 *
 * Cloudflare AI Gateway acts as a proxy in front of upstream AI providers, offering
 * caching, rate limiting, analytics, and logging. This provider uses the OpenAI SDK
 * under the hood (pointed at the Cloudflare AI Gateway URL via `createCloudflareClient`).
...
export class CloudflareProvider implements ModelProvider {
  ...
  /**
   * Creates a new Cloudflare AI Gateway provider instance.
   *
   * @param sdk - An OpenAI SDK client configured for the Cloudflare AI Gateway endpoint (see `createCloudflareClient`).
  ...
  async execute_request(
    ...
    const params: OpenAI.ChatCompletionCreateParamsStreaming = {
      ...
      ...(request.tools.length > 0
        ? {
            tool_choice: "auto",
            tools: request.tools.map((t) => to_cloudflare_tool(t)),
          }
        : {}),
      messages: [
        ...get_system_message(request.system),
        ...request.messages.flatMap((m) => to_cloudflare_messages(m)),
      ],
      stream_options: {
        include_usage: true,
      },
   ...
   */
  clone(): ModelProvider {
    return new CloudflareProvider(this.#sdk, this.#config, this.context_transformers);
  }
  ...
  async #stream_response(
    ...
    try {
      ...
      for await (const response_chunk of stream) {
        ...
        for (const choice_chunk of choices_chunk) {
          if (!response.choices[choice_chunk.index]) {
            ...
            const message = from_cloudflare_completion(response_chunk, choice_chunk);
          ...
          if (delta_chunk.content) {
            if (!choice.delta.content) {
              ...
              receiver.start_content({
                content: from_cloudflare_content(choice.delta) as AssistantContent,
                message: from_cloudflare_completion(response_chunk, choice),
                usage: response?.usage ? from_cloudflare_usage(response.usage) : {},
              });
          ...
          if (delta_chunk.tool_calls) {
            ...
            for (const tool_call_chunk of delta_chunk.tool_calls) {
              if (!choice.delta.tool_calls[tool_call_chunk.index]) {
                ...
                receiver.start_content({
                  content: from_cloudflare_tool_call(tool_call_chunk),
                  message: from_cloudflare_completion(response_chunk, choice),
                  usage: response?.usage ? from_cloudflare_usage(response.usage) : {},
                });
                receiver.update_message({
                ...
                receiver.update_content({
                  content: from_cloudflare_tool_call(tool_call),
                  message: from_cloudflare_completion(response_chunk, choice),
                  usage: response?.usage ? from_cloudflare_usage(response.usage) : {},
                });
                receiver.update_message({
      ...
      const message = from_cloudflare_completion(response, response.choices[0]);
      const usage = response?.usage ? from_cloudflare_usage(response.usage) : {};
      for (const content of message.content) {
...
function to_cloudflare_tool(tool: ToolDefinition): OpenAI.Chat.ChatCompletionTool {
...
function from_cloudflare_completion(
  ...
  for (const k in choice.delta) {
    ...
    if (key === "content" && choice.delta.content) {
      contents = [...contents, from_cloudflare_content(choice.delta)];
    } else if (key === "refusal" && choice.delta.refusal) {
      contents = [...contents, from_cloudflare_refusal(choice.delta)];
    } else if (key === "tool_calls" && choice.delta.tool_calls) {
      contents = [...contents, ...choice.delta.tool_calls.map((t) => from_cloudflare_tool_call(t))];
...
function from_cloudflare_refusal(
  ...
  return {
    ...
    extended: {
      ...rest,
      cloudflare_refusal: true,
    },
  } as Content;
}

function from_cloudflare_content(
  content: OpenAI.Chat.Completions.ChatCompletionChunk.Choice.Delta,
) {
  const {
    content: c,
...
function from_cloudflare_tool_call(
...
function to_cloudflare_messages(message: Message) {
  if (message.role === "assistant") {
    return [to_cloudflare_assistant_message(message)];
  }
  // Partition content so tool_result blocks come before non-tool_result blocks.
  // OpenAI-compatible APIs require all tool-role messages immediately after the assistant message
...
function to_cloudflare_assistant_message(message: AssistantMessage) {
  ...
  for (const content of message.content) {
    switch (content.type) {
      case "text":
        if (
          content.extended &&
          (content.extended.cloudflare_refusal === true || content.extended.openai_refusal === true)
        ) {
      ...
      case "tool":
        ...
        break;
      case "raw":
        // Handle raw content from cloudflare or openai providers (format-compatible)
        if (content.model_kind !== "cloudflare" && content.model_kind !== "openai") {
          if (typeof result.content === "string") {
            result.content = [
              {
                type: "text",
  ...
  // Some OpenAI-compatible backends require the `content` key to be present on
  // assistant messages even when it is null.
...
function from_cloudflare_usage(usage: OpenAI.CompletionUsage | null | undefined) {
  return {
