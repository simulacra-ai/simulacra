/**
 * End-to-end tests for the Cloudflare AI Gateway provider.
 *
 * These tests make real HTTP requests through a Cloudflare AI Gateway.
 * They are skipped automatically when the CLOUDFLARE_API_TOKEN environment variable
 * is not set, so they are safe to run in CI without secrets configured.
 *
 * To run locally:
 *   CLOUDFLARE_API_TOKEN=your_token npm run test:e2e -w packages/cloudflare
 */
...
const PROVIDER = process.env.CLOUDFLARE_PROVIDER || "";
const TEST_MODEL = process.env.CLOUDFLARE_TEST_MODEL || "";
...
async function run_request(
  ...
  const sdk = createCloudflareClient(API_TOKEN as string, {
    accountId: ACCOUNT_ID,
    gatewayName: GATEWAY_NAME,
    provider: PROVIDER,
  });
  const provider = new CloudflareProvider(sdk, config);
...
describe.skipIf(!API_TOKEN)("Cloudflare AI Gateway E2E – simple conversation", () => {
...
describe.skipIf(!API_TOKEN)("Cloudflare AI Gateway E2E – multi-turn conversation", () => {
...
describe.skipIf(!API_TOKEN)("Cloudflare AI Gateway E2E – tool use", () => {
...
describe.skipIf(!API_TOKEN)("Cloudflare AI Gateway E2E – streaming events", () => {
